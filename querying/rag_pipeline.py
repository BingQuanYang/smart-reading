...
class RagPipeline:
    ...
    def __init_llm(self, api_key: str) -> ChatTongyi:
        if self.llm is None:
            self.llm = ChatTongyi(
    ...
    def _init_embeddings(self, api_key) -> DashScopeEmbeddings:
        if self._embeddings is None:
            self._embeddings = DashScopeEmbeddings(model=self.config.embedding_model, dashscope_api_key=api_key)
    ...
    def query(self,
        ...
        llm = self.__init_llm(dashscope_api_key)
        ...
        # 向量召回
        recall_docs, vec_scores = recall_with_scores(search_query, vectorstore, k=recall_k)

        evidence_list = []
        context = ""
        if enable_rerank:
            rerank_pipeline = RerankPipeline(self.config, llm)
            evidence_list, context = rerank_pipeline.rerank(question, recall_docs, vec_scores,
        ...
        answer = answer_generator.generator(search_query, chat_history=chat_history, context=context)
