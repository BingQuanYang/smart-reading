...
class RerankPipeline:
    ...
    def rerank(self, query: str, recall_docs: List[Document], vec_scores: List[float], hybrid_top_m: int = 12,
               final_top_n: int = 4) -> Tuple[List[RetrievedEvidence], str]:
        # 预过滤
        pre_docs = self.__filter(recall_docs)
        # 对齐向量分数(因为过滤可能减少)
        score_map = {doc.page_content: s for doc, s in zip(recall_docs, vec_scores)}
    ...
    def __filter(self, docs: List[Document], min_len=40, max_len=2000) -> List[Document]:
        """
            # ------------------ 预过滤： 长度过滤+ 去重 ------------------
