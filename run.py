...
class PDFQA:
    ...
    def ask(self, question: str) -> dict:
        ...
        if self.file_hash is None:
            self._build_index()
        result = self.rag.query(dashscope_api_key=self.api_key, file_hash=self.file_hash, question=question,
                                chat_history=self.chat_history.messages, enable_rewrite=True, enable_rerank=True,
                                recall_k=30, hybrid_top_m=12, top_k=4)
